...
final class AuthHandler {
    ...
    public function register(Request $req): Response {
        ...
        if (!$v->ok()) return Response::validationFailed('Invalid registration.', $v->errors());
        if ($this->users->findByUsername($username) !== null) {
            return Response::error(409, ErrorCodes::CONFLICT, 'Username already exists.');
        }
    ...
    public function login(Request $req): Response {
        ...
        $vague = Response::error(401, ErrorCodes::UNAUTHENTICATED, 'Username or password is incorrect.');
    ...
    public function me(Request $req, array $params, int $userId): Response {
        ...
        if ($u === null) return Response::error(401, ErrorCodes::UNAUTHENTICATED, 'Session is no longer valid.');
